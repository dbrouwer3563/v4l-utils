...
# trace a nested struct member in videodev2.h
sub handle_struct {
...
sub handle_pointer {
	my $pointer_line = shift;
	printf $fh_trace_cpp "\t//$pointer_line\n";
	printf $fh_retrace_cpp "\t//$pointer_line\n";
...
sub struct_gen {
	...
	while ($line = <> ) {
		...
		# supress anything inside a union including structs nested inside a union
		if ($suppress_union eq true) {
			printf $fh_trace_cpp "\t//$line\n";
			printf $fh_retrace_cpp "\t//$line\n";
			next;
...
sub struct_gen_ctrl {
	...
	while ($line = <> ) {
		...
		if ($line =~ /.*\[.*/) {
			...
			# Add members with a single string value (e.g. enums, defines)
			$val_def_name = get_val_def_name($member, $struct_name);
			if ($val_def_name !~ /^\s*$/) {
				printf $fh_trace_cpp "string(val2s(p->$member, %s).c_str()));\n", $val_def_name;
